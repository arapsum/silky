...
These defaults match `service/config/development.yaml`. If you use different
...
- Health and authentication endpoints, including registration, verification,
  cookie-based login, refresh-token rotation, logout, password recovery, and
  profile management.
- Public category and product reads with pagination and filtering.
- Permission-protected category and product creation, updates, soft deletion,
  variants, default variants, product pictures, and variant pictures.
- Authenticated role and permission reads, role management, role-permission
  assignment, and permission-protected user listing.

See [`service/README.md`](service/README.md#api-routes) for the complete route
...
The admin includes protected, API-backed screens for categories, products,
customers, staff, roles, permissions, and account settings. Product management
covers catalogue filtering, creation, detail, editing, variants, and pictures.
The shared API client retries eligible requests after refreshing an expired
session and redirects to sign-in when authentication cannot be restored.
...
- Admin image uploads go directly to Cloudinary when the two Cloudinary client
  environment variables are configured; the resulting URL is sent to the API.
