...
export function OrderHistory() {
  ...
  return (
    <div>
      ...
      {loading ? <div className="account-state">Loading orders...</div> : orders.length ? <div className="order-list">{orders.map((order) => <a href={`/account/orders/${order.pid}`} key={order.pid}><span><strong>ORD-{String(order.orderNumber).padStart(6, "0")}</strong><small>{new Intl.DateTimeFormat("en", { dateStyle: "medium" }).format(new Date(order.createdAt))}</small></span><span><small>Payment</small><span className="status-pill">{order.paymentStatus}</span></span><span><small>Order</small><span className="status-pill">{order.status}</span></span><strong>{formatCurrency(order.grandTotal, order.currency)}</strong><ArrowRightIcon aria-hidden size={16} /></a>)}</div> : <p className="empty-copy">No orders yet. Your first one will appear here.</p>}
